...
print"User's search: ".$searchText;
    ...
    $SQLselect = "SELECT * FROM " . $databaseTable." WHERE MATCH(style,author,title,authorImage,bookImage,authorBio,isbn,pubCo) "."AGAINST ('".$searchText."' IN NATURAL LANGUAGE MODE)";
    ...
    if ($numrecs > 0) {
        if ($firstrec == true){
            ...
            while ($recordArray = mysqli_fetch_row($results)) {
                ...
                if ($imagenumber <= 5) {
                        print"<td><a href='bookinfo_search.php?".htmlentities($url)."'><img src='".$bkImage."' width='100px'height='150px'/></a></td>";
                    $imagenumber++;
